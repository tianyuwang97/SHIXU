// Rule 2: fixed 15-calendar-day setup, two consecutive published observations.
...
const above=(value,level)=>value>level*(1+1e-12);
function evaluateBreakout(row,context,previous=null){
 ...
 for(const p of fresh){
  ...
  if(state.phase==='pending'){
   if(above(value,state.box.upper)){
    state.phase='confirmed';state.box.confirmDate=p.date;
    state.events.push({type:'buy',date:p.date,id:state.box.id});
   }else{
   ...
   if(start>=state.trackingStart&&start>=state.samples[0].date){
    ...
    if(setup.length>=2&&expected.every(d=>setup.some(s=>s.date===d))){
     ...
     if(amplitude<=.1+1e-12&&Math.abs(change)<=.05+1e-12&&above(value,upper)){
 ...
 return {state,result:{known:true,phase:state.phase,asof:context.asof,trackingStart:state.trackingStart,box,newBuy:state.phase==='confirmed'&&box?.confirmDate===context.asof,newReview:state.phase==='exited'&&box?.exitDate===context.asof}};
}
