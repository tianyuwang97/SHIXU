...
export function stockExit(stock,context){
 ...
 if(!points.length||dates.at(-1)!==asof||dates.some((d,i)=>i&&d<=dates[i-1])||points.some(p=>!Number.isFinite(p[1])||p[1]<=0)||calendar.some(d=>d<=asof&&!dates.includes(d))||historyStart>start)return unknown('行情缺失、停牌或30自然日历史不足');
 ...
 const box=result.box?{...result.box,upper:result.box.upper*scale,lower:result.box.lower*scale}:null;
 return {...base,status:matched?'triggered':'clear',asof,start,amplitude,change,rule1,rule2,phase:result.phase,
  ...
  ...(matched?{latest:points.at(-1)[1],box,points:points.filter(p=>p[0]>=box.rangeStart),explanation:`近30自然日振幅 ${(amplitude*100).toFixed(2)}%，首尾涨跌幅 ${(change*100).toFixed(2)}%。首次跌破前锁定 ${box.rangeStart}—${box.rangeEnd} 的15自然日横盘区间；${box.firstBreakDate}首次跌破原下沿，${box.confirmDate}第二个连续收盘点确认，截至${asof}未涨回。区间下沿不随后续走势移动。`}:{explanation:'截至当前，未同时符合30日横盘与向下突破确认条件。'})};
}
...
export function buildFundExits(daily){
 ...
 const stocks=daily.rows.map(row=>{
  ...
  const points=row.history.map((p,i)=>{if(!Number.isFinite(p[1])||p[1]<=0||i&&(p[3]||!Number.isFinite(p[2]||0)))error='净值、分红或拆分口径待核实';if(i)value*=(p[1]+(p[2]||0))/row.history[i-1][1];return [shift(c.history_start,p[0]),value];});
  return {code:row.code,name:row.name,source:row.source,points,error};
 });
 return buildStockExits({market:'FUND',asof:c.asof,historyStart:c.history_start,calendar:c.dates,currency:'CNY',priceBasis:'分红再投净值指数（历史首点=1）',stocks});
}
