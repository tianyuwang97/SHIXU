...
export function initExitMode(onChange){
 ...
 function paint(){
  document.querySelector('.decision-caption').classList.remove('has-invitation');
 ...
 async function change(next,trigger=null){
  ...
  try{
   ...
   $('decisionHint').textContent='暂时无法验证登录，请稍后再试。';
   ...
   if(active)$('exitTitle').focus({preventScroll:true});else (trigger||document.querySelector('[data-decision="exit"]')).focus({preventScroll:true});
   if(guestReturn&&!active){
    $('decisionHint').setAttribute('role','status');
 ...
 async function load(){
  ...
  try{const response=await fetch('/api/stock-exits?'+new URLSearchParams({market,...(scope==='holdings'?{codes:holdings().map(r=>r.code).join(',')}:{})}),{cache:'no-store',credentials:'same-origin',signal:AbortSignal.timeout(20000)});if(response.status===401){if(id===version&&active){active=false;paint();persist();onChange();$('decisionHint').innerHTML='登录已失效，请重新登录。 <a href="'+loginURL()+'">去登录</a>';document.querySelector('.decision-caption').classList.add('has-error');}return;}if(!response.ok)throw Error();const next=await response.json();if(next.market!==selected.toUpperCase()||!Array.isArray(next.rows))throw Error();if(id!==version||!active)return;data=next;render();}catch{if(id!==version||!active)return;$('exitMeta').textContent='退出检查暂未载入';$('exitCards').innerHTML='<div class="exit-empty"><p>暂时无法读取行情，请重试。</p><button id="retryExit">重新加载</button></div>';$('retryExit').onclick=load;}
 }
